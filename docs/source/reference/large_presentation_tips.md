...
6. [Minimise TeX Calls](#minimise-tex-calls)
...
presentation content and timing are finalised.
...
time — one per CPU core.  There is one important caveat: generated artefacts such
as TeX images will collide if every scene writes to the same directory.  Give
each scene its own `media_dir` to avoid this:
...
class ExplainConcepts(Slide):
    ...
    class Config:
        media_dir = "media_explain_concepts"
```

With isolated media directories you can safely fan out rendering across as many
threads or processes as you have cores.  After all jobs finish, run your
...
> artefacts are baked into the output.
...
not use reversed animations — such as PPTX or HTML — this is pure wasted
rendering time.
...
Even when reversed slides *are* enabled, their generation can sometimes hang
indefinitely.  The root cause is an internal parallelisation step that is prone
to deadlocking.  A known workaround is to disable that parallelisation by setting
