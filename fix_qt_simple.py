...
with open("qt_errors.txt", encoding="utf-16") as f:
    content = f.read()
...
for match in all_matches:
    ...
    error_context = content[
        content.find(f"--> {filepath}:{line_num}") : content.find(
            f"--> {filepath}:{line_num}"
        )
        + 500
    ]
    if (
        "PyQt6" in error_context
        or "PySide6" in error_context
        or "QWidget" in error_context
        or "QLayout" in error_context
    ):
